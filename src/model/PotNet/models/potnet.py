...
from src.model.PotNet.models.base import BaseSettings
from src.model.PotNet.models.transformer import TransformerConv

from src.model.PotNet.models.utils import RBFExpansion
...
class PotNet(nn.Module):
    ...
    def __init__(self, config: PotNetConfig = PotNetConfig(name="potnet")):
        ...
        self.fc = nn.Linear(config.fc_features, config.embedding_dim)
        if config.projector:
            self.projector = nn.Linear(config.embedding_dim, config.embedding_dim)
        if config.final_bn:
            self.final_bn = nn.BatchNorm1d(config.embedding_dim)

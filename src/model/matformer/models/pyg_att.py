...
from src.model.matformer.models.utils import RBFExpansion
from src.model.matformer.utils import BaseSettings
from src.model.matformer.features import angle_emb_mp
from torch_scatter import scatter
from src.model.matformer.models.transformer import MatformerConv
...
class Matformer(nn.Module):
    ...
    def forward(self, data) -> torch.Tensor:
        ...
        if self.angle_lattice: ## module not used
            ...
            cos2 = self.lattice_angle(torch.clamp(torch.sum(lattice[:,1,:] * lattice[:,2,:], dim=-1) / (torch.norm(lattice[:,0,:], dim=-1) * torch.norm(lattice[:,2,:], dim=-1)), -1, 1).unsqueeze(-1)).view(-1, 128)

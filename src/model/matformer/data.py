...
def get_pyg_dataset(
    ...
    """Get pyg Dataset."""
    df = pd.DataFrame(dataset)
    # construct the dataframe from all of the available information
    vals = df[target].values
    if target == "shear modulus" or target == "bulk modulus":
        val_list = [vals[i].item() for i in range(len(vals))]
        vals = val_list
...
def get_train_val_loaders(
    ...
    if (
        ...
        if not dataset_array:
            d = jdata(dataset)
            # print(d[0]['id'], d[0]['atoms'].keys())
            # print(d[0]['atoms']['lattice_mat'],
            #       d[0]['atoms']['coords'], d[0]['atoms']['elements'],
            #       d[0]['atoms']['abc'])
            # exit()
        else:
            d = dataset_array

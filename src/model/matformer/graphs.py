    ...
    def __init__(
        ...
        if save_data:
            ...
            for i in range(len(self.graphs)):
                material_id = self.df.loc[i, 'id']
                crystal_megnet[material_id] = self.graphs[i]
                target_megnet[material_id] = self.labels[i]
            
            num_materials = len(self.graphs)
            task = 'bandgap' if target == 'gap pbe' else target
            torch.save(crystal_megnet, f'./data/data_reproduce_matformer/crystal_megnet_{num_materials}.pt')
            torch.save(target_megnet, f'./data/data_reproduce_matformer/{task}_megnet_{num_materials}.pt')
            print('Saved dicts to Soljacic shared folder')
